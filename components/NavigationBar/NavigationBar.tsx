  ...
  return (
    <nav className={`${type === 'classic' ? styles.nav : styles['nav-admin']} ${isHero ? styles.heroNav : styles.boxShadow} ${shouldHide ? styles.hidden : ""}`}>
      {type === 'admin' && user ? (<>
        ...
        <div className={`${styles.search} ${styles['nav-search']}`}>
        ...
        <button className={styles.hamburger} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
          <svg viewBox="0 0 100 80" width="30" height="30">
            <rect width="100" height="15" fill="#4C4C4C" rx="8"></rect>
            <rect y="30" width="100" height="15" fill="#4C4C4C" rx="8"></rect>
            <rect y="60" width="100" height="15" fill="#4C4C4C" rx="8"></rect>
          </svg>
        </button>
        <div className={`${styles['admin-nav-content']} ${isMobileMenuOpen ? styles['menu-open'] : ''}`}>
          <div className={styles['mobile-menu-links']}>
            {Object.keys(icons).map((key) => (
              <Link 
                key={key} 
                href={key === 'Dashboard' ? '/admin' : `/admin/${key.toLowerCase()}`}
                className={`${styles['mobile-menu-link']} ${pathname === (key === 'Dashboard' ? '/admin' : `/admin/${key.toLowerCase()}`) ? styles.active : ''}`}
