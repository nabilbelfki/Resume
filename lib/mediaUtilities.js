...
export async function handleFileUpload(req, existingMedia = null) {
  const form = new IncomingForm({
    keepExtensions: true,
    maxFileSize: 5 * 1024 * 1024 * 1024, // 5GB Max Size
    multiples: false,
  ...
  const allowedForType = ALLOWED_EXTENSIONS[fileType] || ALLOWED_EXTENSIONS.image;
  if (!allowedForType.includes(ext)) {
    // Delete the file from tmp since it's rejected
    await fs.unlink(file.filepath).catch(() => { });
    throw new Error(`Invalid file extension (${ext}). Allowed extensions for ${fileType} are: ${allowedForType.join(', ')}`);
  }
